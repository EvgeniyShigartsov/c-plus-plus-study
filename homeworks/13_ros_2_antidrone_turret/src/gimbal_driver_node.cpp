...
public:
  ...
  {
    subscription_ =
      create_subscription<GimbalCommand>(kGimbalCommandTopic, 10, [this](const GimbalCommand& command) { on_command(command); });

    RCLCPP_INFO(get_logger(), "listening for gimbal commands on %s", kGimbalCommandTopic);
  }
...
private:
  void on_command(const GimbalCommand& command)
  {
    RCLCPP_INFO(get_logger(),
                "gimbal command direction=%s target_y=%.1f error_y=%.1f",

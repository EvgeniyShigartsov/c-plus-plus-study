...
bool readTargets(float out_targetXInTime[TARGETS_COUNT][TARGET_MOVES_COUNT], float out_targetYInTime[TARGETS_COUNT][TARGET_MOVES_COUNT])
{
...
void setInterpolationIndex(const float t, const float arrayTimeStep, int& out_idx, int& out_next, float& out_frac)
{
...
void setFirePoint(
  const float targetX, const float targetY, const float xd, const float yd, const float h, float& out_fireX, float& out_fireY)
{
...
void updateDroneXY(const float CURRENT_DIR, const float CURRENT_SPEED, const float simTimeStep, float& out_droneX, float& out_droneY)
{
...
int main()
{
  ...
  if (!readInput(
        xd, yd, zd, initialDir, v0, accelerationPath, ammo_name, arrayTimeStep, simTimeStep, hitRadius, angularSpeed, turnThreshold)) {
    return 1;
  }
  ...
  while (step <= MAX_STEPS && !reachedFirePoint) {
    ...
    for (int i = 0; i < TARGETS_COUNT; i++) {
      ...
      if (i != selectedTargetIndex) {
        ...
        const float deltaAngle = fabs(dirToFire - CURRENT_DIR);
    ...
    if (deltaAngle > turnThreshold) {
      if (CURRENT_STATE == MOVING || CURRENT_STATE == ACCELERATING)
    ...
    if (CURRENT_STATE == DECELERATING) {
      ...
      dirToFire > CURRENT_DIR ? CURRENT_DIR += angularSpeed* simTimeStep : CURRENT_DIR -= angularSpeed * simTimeStep;
